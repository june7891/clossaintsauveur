<footer class="text-center text-lg-start" style="background-color: rgba(230, 232, 212, 1);">
    <div class="pt-5 footer-info">
        <a href="<?= URL ?>"><img src="assets/images/logo.png" alt="logo"></a>
        <ul>
            <li> <i class="fa-solid fa-location-dot"></i>
                <p>8 route de la Magdelaine, Saint-Pierre-d'Aurillac, France</p>
            </li>
            <li><i class="fa-solid fa-phone"></i>
                <p>06 10 88 15 66</p>
            </li>
            <li><i class="fa-solid fa-envelope"></i>
                <p>clossaintsauveur33@gmail.com</p>
            </li>
            <li>
                <div class="social-icons">
                    <a href="https://www.facebook.com/clossaintsauveur"><i class="fa-brands fa-facebook"></i></a>
                    <a href=""><i class="fa-brands fa-square-instagram"></i></a>
                </div>
            </li>
        </ul>

        <ul>
            <li> <a href="<?= URL ?>legal"><?php echo $lang['legal']?></a> </li>
            <li> <a href="<?= URL ?>CGV"><?php echo $lang['CGV']?></a> </li>
        </ul>

    </div>

    <div class="text-center p-3">
        © 2023 Created by :
        <a class="text-dark" href="https://juunee-concept.com" target="_blank">Juunee Concept</a>
    </div>

</footer>